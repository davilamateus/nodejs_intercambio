    ...
    Users.findOne({where:{email:email}}).then((data)=>{
        Forget.create({token:uuid.v4(), userId:data.id, status:false}).then((result)=>{
            ...
            const transporter = nodemailer.createTransport({
                ...
                port: 465,
                auth: {
                  user: "no-reply2@mateusdavila.com", 
                  pass: "Goias123.", 
                },
              });
              let info =  transporter.sendMail({
                from: "no-reply2@mateusdavila.com",
                to: `${email}`, 
                ...
                <p style="text-align: center;"><a href="https://mateusdavila.com/forget-password/${result.token}">https://mateusdavila.com/confirm-email/${result.token}</a></p>
